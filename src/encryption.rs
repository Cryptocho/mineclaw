...
impl EncryptionManager {
    ...
    pub fn decrypt(&self, ciphertext_base64: &str) -> Result<String> {
        ...
        if combined.len() < 28 {
            return Err(anyhow!(
                "Ciphertext too short (min 28 bytes required for Nonce+Tag)"
            ));
        }
        ...
        let mut plaintext_bytes = cipher.decrypt(nonce, ciphertext_with_tag).map_err(|_| {
            anyhow!("Decryption failed: Tag verification failed or ciphertext corrupted")
        })?;

        let plaintext =
            String::from_utf8(plaintext_bytes.clone()).context("Invalid UTF-8 plaintext")?;
