...
- Agent 意识到自己无法解决的问题（如权限限制、资源不足、需要人工决策等）
- Agent 直接发求助工单给 Master
- Master 分析后决定如何处理（回退、换人、调整资源、或其他）

**路径 2：上下文满载时 CMA 自动处理（唯一的自动触发点）**
- 触发条件：当前上下文长度 ≥ 阈值（如 60%，自适应调整）
- **步骤 1：CMA 检测是否持续犯错**
  - 分析最近的上下文，检测重复的错误模式
  - 如果检测到持续犯错（≥3次相同错误）→ 进入步骤 2
  - 如果只是普通上下文积累 → 只做裁剪，返回
- **步骤 2：CMA 判断是否可以回退**
  - 如果有可用的 Checkpoint → CMA 执行 rollback + 注入错误总结 + 让 Agent 继续（自我纠正）
  - 如果无法回退（如缺少依赖、无法恢复的错误）→ CMA 代发工单给 Master

### 6.1 CMA 代发工单的条件（权限范围外的事）

**CMA 只在以下情况代发工单给 Master**：
- 缺少项目依赖（如 `npm install`、`cargo add` 等无法自动完成的依赖）
- 需要调整资源限制（如内存、并发数等 Agent 无法自行调整的配置）
- 需要人工决策（如选择技术方案、权衡取舍等）
- 其他 Agent 权限范围外必须 Master 介入的事项

**注意**：普通代码错误（逻辑错误、拼写错误等）不在此列，这类错误应该让 Agent 通过 rollback 自我纠正。
