...
**请求：**
```bash
curl -X POST http://127.0.0.1:18789/api/messages/stream \
...
**或使用会话 ID：**
```bash
curl -N http://127.0.0.1:18789/api/sessions/{session-id}/stream
```
...
- [ ] 修改 ToolCoordinator 支持回调（每轮触发）
- [ ] 使用 `tokio::sync::mpsc` 或 `broadcast` 传递事件
